...
export class BookService {
  ...
  borrowBook(id:string,headers:HttpHeaders): Observable<any> {
    console.log(headers);
    return this.http.post(this.serverUrl+`/transaction`,{id:id},{headers});
  };

  returnBook(id:string,headers:HttpHeaders): Observable<any> {
    console.log(headers);
    return this.http.post(this.serverUrl+`/return`,{book_id:id},{headers});
  };

  review(id:string,rating:Number,review_author:string,review:string, headers:HttpHeaders): Observable<any> {
    return this.http.post(this.serverUrl+`/review`,{book_id:id,rating,review_author,review},{headers});
  };
